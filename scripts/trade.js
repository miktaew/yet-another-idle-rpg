    ...
    this.get_inventory_from_template = function() {
        ...
        for(let i = 0; i < inventory_template.length; i++) {
            if(inventory_template[i].chance >= Math.random()) {
                ...
                if(inventory_template[i].stackable) { //stackable
...
inventory_templates["Basic"] = new Inventory_template({inventory_template :
    [
        new Trade_item({item_name: "Long stick", count: [2]}),
        new Trade_item({item_name: "Plank with a handle", count: [2,5]}),
        new Trade_item({item_name: "Crude wooden shield", count: [1], chance: 0.5}),
        new Trade_item({item_name: "Shabby leather vest", count: [1,3]}),
        new Trade_item({item_name: "Raggy leather pants", count: [1,3]}),
        new Trade_item({item_name: "Rusty knife", count: [1,3]}),
